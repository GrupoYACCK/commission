...
class ProductProduct(models.Model):
    ...
    def get_commission_by_product(self, agent_id):
        ...
        commission_id = self.env['product.commission.agent'].search([('product_id','=',self.product_tmpl_id.id),
                                                                     ('agent_id','=', agent_id)], limit = 1).commission_id.id
        if not commission_id:
            commission_id = self.env['product.commission.agent'].search([('product_id','=',self.product_tmpl_id.id)], limit = 1).commission_id.id
